...
class Alfredo(PipelineEnv):
  ...
  def step(self, state: State, action: jp.ndarray) -> State:
    ...
    dist_diff = jp.array([pipeline_state.q[-2] - com_after[0],
                          pipeline_state.q[-1] - com_after[1]])
  ...
  def _com(self, pipeline_state: base.State) -> jp.ndarray:
    ...
    com = (
            jp.sum(jax.vmap(jp.multiply)(inertia.mass[:-1], 
                                         x_i.pos[:-1]), axis=0) / mass_sum
    )
